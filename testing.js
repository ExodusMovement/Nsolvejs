...
commander.option('-a, --alpha [n]', 'Value of alpha', parseFloat).parse(process.argv);
...
gnuplot().set('term png').set('output ' + fileplot).set('title "Plotdata"').set('xlabel "Time"').set('grid').set('ylabel "Size"').set('key left Left reverse spacing 6 font ",11"').set('zeroaxis').set('autoscale').plot('"./plots/plot.dat" ps 2 pt 3 t "Data",' + '"./plots/plot_smt.dat" ps 1 pt 5 t "Data-smt",' + function_plot(fit.fitUsed, fit.fitParamsUsed) + '  t "Bestfit=' + function_plot(fit.fitUsed, fit.fitParamsUsed) + '","./plots/plot_ansy.dat"  ps 2 pt 7 lc rgb "blue" t "ans-ofY",' + '"./plots/plot_ansx.dat" ps 2 pt 13 lc rgb "black" t "ans-ofX"').set(' output').end();
//console.log(
