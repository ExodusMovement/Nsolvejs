...
describe( 'JNsolve Module numeric values function test.', function () {
	...
	it( 'Found the correct solution to cos(x)-x=0 is 0.73 using the regulafalsi method.', function () {
		assert.equal( JNsolve.calculusN.regulafalsi( f, interval )
	...
	it( 'The adjunted matrix should be a matrix with (2,1) component equal to -1', function () {
		assert.equal( JNsolve.AL.matrix.adj( A )
			._( 2, 1 ), -1 ); // should returns true
	} );
	it( 'The matrix method ones generates a matrix with only ones', function () {
		assert.equal( JNsolve.AL.matrix.ones( 6, 8 )
			._( 6, 8 ), 1 ); // should returns true
	} );
	it( 'The matrix method zeros generates a matrix with only zeros', function () {
		assert.equal( JNsolve.AL.matrix.zeros( 15, 18 )
			._( 3, 11 ), 0 ); // should returns true
	} );
	it( 'The product direct of A with itself has 1,2 component equal 1 ', function () {
		assert.equal( A._x( A )
			._( 1, 2 ), 16 ); // should returns true
	} );it( 'The apply method with matrix [[cos,sin],[tan,sqrt]] return a matrix with the correct array', function () {
			var matFunc = JNsolve.AL.matrix( [ [ Math.cos, Math.sin ], [ Math.tan, Math.sqrt ] ] )
			var matApply = matFunc.apply( A )
			assert.equal(
				matApply._( 1, 1 ), Math.cos( A._( 1, 1 ) ) ); // should returns true

			assert.equal(
				matApply._( 1, 2 ), Math.sin( A._( 1, 2 ) ) ); // should returns true

			assert.equal(
				matApply._( 2, 1 ), Math.tan( A._( 2, 1 ) ) ); // should returns true

			assert.equal(
				matApply._( 2, 2 ), Math.sqrt( A._( 2, 2 ) ) ); // should returns true
	...
	it( 'When the object {a:21,b:"hola"} is filtered with the array = [0,1] and the method toObject is applied return {b:"hola"}', function () {
		assert.equal( JNsolve.AL.matrix({a:21,b:'hola',c:34,d:'eitt',key:'value'}).filter([0,1,true,false]).toObject().b,'hola'); // should returns true
	} );
...
describe( 'derivative numeric.', function () {
	...
	it( 'JNsolve.D.deltax return aobject with property x_n_array like a Array', function () {
		assert.equal( Array.isArray( JNsolve.calculusN.D.deltax( 100, [ 0, 123 ] )
	...
	it( 'JNsolve.D.derivativenumeric return a object with property dfdx_array like a Array', function () {
		assert.equal( Array.isArray( JNsolve.calculusN.D.derivativenumeric( f, 100, [ 0, 123 ] )
	...
	it( 'JNsolve.D.linearinterapolation is a constructor that define the function_interpolated', function () {
		assert.equal( typeof new JNsolve.calculusN.D.linearinterapolation( [ 3, 2 ], [ 6, 8 ], [ 2, 9 ] )
...
describe( 'Negative cases.', function () {
	...
	it( 'If root of function given is complex or does not exist, return undefined.', function () {
		assert.equal( JNsolve.calculusN.fixedpoint( function ( x ) {
			...
			.Root, undefined ); // should returns true
	} );
	...
	it( 'If the initial point is  far away interval that contain the solution to cos(x)-x=0 is 0.73  using the Newton_Raphson method do not converge.', function () {
		assert.equal( JNsolve.calculusN.Newton_Raphson( f, [ -3, 1 ], 7 )
			.Root, undefined ); // should returns true
	} );
	...
	it( 'If interval does not contain the solution to cos(x)-x=0 is 0.73 using the Newton_Raphson-Higher Order method the solution is not found.', function () {
		assert.equal( JNsolve.calculusN.Newton_Raphson_Higherorder( f, [ 3, 10 ], initialpoint )
			.Root, undefined ); // should returns true
	} );
...
describe( 'Stats module test', function () {
	...
	it( 'the media of datas given is [ [ 3.2857142857142856 ],[ 4 ],[ 3.2857142857142856 ],[ 3.714285714285714 ],[ 0.7142857142857142 ],[ 5 ],[ 5.428571428571428 ] ] ',
		function () {
			assert.equal( stats.media()
				._( 1, 1 ), 3.2857142857142856 );
			assert.equal( stats.media()
				._( 2, 1 ), 4 );
			assert.equal( stats.media()
				._( 3, 1 ), 3.2857142857142856 );
			assert.equal( stats.media()
				._( 4, 1 ), 3.714285714285714 );
			assert.equal( stats.media()
				._( 5, 1 ), 0.7142857142857142 );
			assert.equal( stats.media()
				._( 6, 1 ), 5 );
			assert.equal( stats.media()
				._( 7, 1 ), 5.428571428571428 );
			/// should returns true
		} );

	it( 'the matrix correlation is given by:',
		function () {
			assert.equal( stats.std()
				.diagonal()
				._( 1, 1 ), 2.288688541085317 );
			assert.equal( stats.std()
				.diagonal()
				._( 2, 1 ), 0 );
			assert.equal( stats.std()
				.diagonal()
				._( 3, 1 ), 2.3603873774083293 );
			assert.equal( stats.std()
				.diagonal()
				._( 4, 1 ), 2.627691364061218 );
			assert.equal( stats.std()
				.diagonal()
				._( 5, 1 ), 0.48795003647426666 );
			assert.equal( stats.std()
				.diagonal()
				._( 6, 1 ), 0 );
			assert.equal( stats.std()
				.diagonal()
				._( 7, 1 ), 3.4572215654165053 );
			/// should returns true
		} );
	it( 'the matrix correlation is given by:[ [ 0.4369314487526515 ],[ NaN ],[ 0.42365927286816174 ],[ 0.38056219755369364 ],[ 2.0493901531919194 ],[ NaN ], [ 0.2892496130428152 ] ]',
		function () {
			assert.equal( stats.covariance()
				.diagonal()
				._( 1, 1 ), 0.4369314487526515 );
			assert.equal( stats.covariance()
				.diagonal()
				._( 3, 1 ), 0.42365927286816174 );
			assert.equal( stats.covariance()
				.diagonal()
				._( 4, 1 ), 0.38056219755369364 );
			assert.equal( stats.covariance()
				.diagonal()
				._( 5, 1 ), 2.0493901531919194 );
			assert.equal( stats.covariance()
				.diagonal()
				._( 7, 1 ), 0.2892496130428152 );
			/// should returns true
		} );
} );
